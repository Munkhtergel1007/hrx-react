...
import config, {isPhoneNum, uuidv4, isRegisterId} from "../../config";
...
class CreateEmployee extends React.Component {
    ...
    submitUser(vals){
        if(
            ...
            obj.register_id = (obj.register_id || '').trim();
            obj.username = (obj.username || '').trim();
            if(obj.email) obj.email = (obj.email || '').trim();
            if(obj.phone) obj.phone = (obj.phone || '').trim();
            ...
            config.get('emitter').emit('error', 'Мэдээллээ шалгана уу.');
        }
    }
    searchUser(vals){
        ...
        // }
        if(!(vals || [])["user-first_name"] || ((vals || [])["user-first_name"] || '').trim() === ''){
            config.get('emitter').emit('error', 'Хайх нэр буруу байна.');
        } else if(!(vals || [])["user-register_id"] || !isRegisterId((vals || [])["user-register_id"])){
            config.get('emitter').emit('error', 'Хайх регистрийн дугаар буруу байна.');
        } else {
            this.props.dispatch(findUser({
                first_name: (vals || [])["user-first_name"],
                last_name: (vals || [])["user-last_name"],
                register_id: (vals || [])["user-register_id"],
            }));
        }
    }
    createEmployee(user){
                ...
                <Col span={18}>
                    ...
                    >
                        ...
                        <Form.Item
                            ...
                            rules={[
                                {
                                    required: (this.state.first_name || '').trim() === '',
                                    message: 'Нэр оруулна уу!',
                                },
                            ]}
                            initialValue={this.state.first_name}
                        >
                            <Input onChange={(e) => this.setState({first_name: e.target.value})} placeholder="Нэр" />
                        </Form.Item>
                        <Form.Item
                            label="Регистрийн дугаар"
                            name="user-register_id"
                            rules={[
                                {
                                    required: !isRegisterId(this.state.register_id),
                                    message: 'Регистрийн дугаар оруулна уу!',
                                },
                            ]}
                            initialValue={this.state.register_id}
                        >
                            <Input onChange={(e) => this.setState({register_id: e.target.value})} placeholder="Регистрийн дугаар" />
                        </Form.Item>
                        <Col span={24} style={{ textAlign: 'right' }}>
                            <Button
                                htmlType="submit"
                                ...
                                loading={findingUser}
                                disabled={
                                    creatingEmp || findingUser ||
                                    (this.state.first_name || '').trim() === '' ||
                                    (this.state.register_id || '').trim() === ''
                                }
                            >
                                Хайх
                            </Button>
                    ...
                    <div style={{marginTop: 60}}>
                        ...
                        {
                            createdUser.exists ?
                                createdUser.email && createdUser.email.toLowerCase() === (createdUser.user || {}).email.toLowerCase() ?
                                    <React.Fragment>
                                        <Alert
                                    ...
                                    : createdUser.phone && createdUser.phone === (createdUser.user || {}).phone ?
                                    <React.Fragment>
                                        <Alert
                                            style={{marginBottom: 10}}
                                            showIcon
                        ...
                        >
                            ...
                            <Form.Item
                                ...
                                rules={[
                                    ...
                                    {
                                        min: 4,
                                        message: 'Хэрэглэгчийн нэвтрэх нэр багадаа 4 үсэг, тооноос тогтсон байх ёстой!',
                                    },
                                    {
                                    ...
                                    {
                                        max: 12,
                                        message: 'Хэрэглэгчийн нэвтрэх нэр ихдээ 12 үсэг, тооноос тогтсон байх ёстой!',
                                    },
                                ]}
